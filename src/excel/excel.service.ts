...
export class ExcelService {
  ...
  private readonly DIST_TEMPLATES_DIR = path.join(
    process.cwd(),
    'dist',
    'excel',
    'templates',
  );
  private readonly SRC_TEMPLATES_DIR = path.join(
    process.cwd(),
    'src',
    'excel',
    'templates',
  ...
  private readonly CLIENT_COLUMNS_TO_CHECK = [
    'A',
    'C',
    'E',
    'G',
  ...
  private readonly CONTROL_PRODUCT_COLUMNS_TO_CHECK = [
    'A',
    'B',
    'D',
    'F',
  ...
  private readonly DISPATCH_PRODUCT_COLUMNS_TO_CHECK = [
    'A',
    'B',
    'D',
    'E',
   ...
   */
  async createNewFileExcel(
    template: TemplatesExcel,
    date?: Date,
  ): Promise<string> {
    ...
    try {
      ...
      this.logger.error(
        `Error al copiar archivo: ${error.message}`,
        error.stack,
      );
   ...
   */
  async addContentRawClient(
    clientExcelDto: ClientExcelDto,
    fileName?: string,
  ): Promise<boolean> {
    const {
      batch_of_product,
    ...
    try {
      ...
      const filePath = await this.pathTempFile(
        fileName,
      ...
      let sheet = await this.getOrCreateSheet(
        workbook,
        TemplatesExcel.MontatyClients,
      );
      ...
      this.logger.error(
        `Error escribiendo datos del cliente en Excel: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async addContentRawControlProduct(
    ...
    try {
      ...
      this.logger.error(
        `Error escribiendo datos de control de producto en Excel: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async addContentRawDispatchProduct(
    ...
    try {
      ...
      this.logger.error(
        `Error escribiendo datos de despacho de producto en Excel: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async pathTempFile(
    ...
    if (fileName) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        this.logger.warn(
          `Archivo específico no encontrado: ${fileName}, buscando alternativas...`,
        );
      }
    }
    ...
    this.logger.log(
      `Buscando archivo para el mes ${month}/${year} con patrón: ${monthFilePattern}`,
    );
    ...
    try {
      ...
      if (monthFile) {
        ...
        this.logger.log(
          `No se encontró archivo para el mes ${month}/${year}, creando uno nuevo...`,
        );

        // Crear un nuevo archivo desde la plantilla para el mes actual
        const newFileName = await this.createNewFileExcel(
          templateType,
          currentDate,
        );
        this.logger.log(
          `Nuevo archivo creado para ${month}/${year}: ${newFileName}`,
        );
      ...
      this.logger.error(
        `Error al buscar/crear archivo para el mes actual: ${error.message}`,
        error.stack,
      );
      throw new Error(
        `Error al buscar/crear archivo para el mes actual: ${error.message}`,
  ...
  async clearTempFiles(): Promise<void> {
    try {
      ...
      this.logger.error(
        `Error al limpiar archivos temporales: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  private async ensureTempDirectoryExists(): Promise<void> {
    if (!fs.existsSync(this.TEMP_DIR)) {
      ...
      try {
        ...
        this.logger.error(
          `Error al crear directorio temporal: ${error.message}`,
          error.stack,
        );
        throw new Error(
      ...
      this.logger.warn(
        'La ruta existe pero no es un directorio, renombrando...',
      );
      try {
        ...
        await fs.promises.mkdir(this.TEMP_DIR, { recursive: true });
        this.logger.log(
          `Archivo renombrado a ${backupPath} y directorio creado exitosamente`,
        );
      } catch (error) {
        this.logger.error(
          `Error al renombrar archivo y crear directorio: ${error.message}`,
          error.stack,
        );
        throw new Error(
          `No se pudo crear el directorio temporal: ${error.message}`,
  ...
  private parseDateFromInput(dateInput: any): Date {
    ...
    try {
      ...
      this.logger.warn(
        `Error al parsear fecha "${dateInput}", usando fecha actual: ${error.message}`,
      );
    }
  ...
  private async getOrCreateSheet(
    ...
    if (sheet) {
      ...
      this.logger.log(
        'No se encontró hoja con espacio disponible, creando una nueva...',
      );
  ...
  private updateEmissionDate(sheet: ExcelJs.Worksheet): void {
    try {
      ...
      if (!contentDate) {
        ...
        this.logger.log(
          `Fecha de emisión actualizada: ${valueEmissionCell.value}`,
        );
      }
    } catch (error) {
      this.logger.error(
        `Error al actualizar fecha de emisión: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  private async findFirstEmptyRow(
    ...
    if (nextAvailableRow <= endRow) {
      this.logger.log(
        `Siguiente fila disponible: ${nextAvailableRow} en hoja: ${sheet.name}`,
      );
      ...
      this.logger.log(
        `Hoja actual ${sheet.name} está llena, creando una nueva hoja...`,
      );
      ...
      // Crear nueva hoja a partir de la plantilla
      const newSheet = await this.copyTemplateSheetFromFile(
        templatePath,
        workbook,
      );
      ...
      this.logger.log(
        `Se alcanzó el límite de filas (${endRow}), usando la primera fila`,
      );
  ...
  private findSheetWithAvailableRow(
    ...
    for (const sheet of workbook.worksheets) {
      ...
      for (
        let rowNum = this.DEFAULT_START_ROW;
        rowNum <= this.DEFAULT_END_ROW;
        rowNum++
      ) {
        ...
        if (this.isCellEmpty(cell)) {
          foundEmptyRow = true;
          this.logger.log(
            `Encontrada fila vacía ${rowNum} en hoja: ${sheet.name}`,
          );
          break;
        }
      }
  ...
  private async copyTemplateSheetFromFile(
    ...
    const now = new Date();
    const formattedDate = now.toISOString().replace(/[:.]/g, '-'); // Ej: 2025-05-15T14-23-07-123Z
    const newSheetName = `Hoja_${formattedDate}`;
    const newSheet = targetWorkbook.addWorksheet(newSheetName);

    // Copiar propiedades de la hoja
    newSheet.properties = {
      ...templateSheet.properties,
      tabColor: templateSheet.properties?.tabColor,
    };

    // Establecer un ancho fijo para todas las columnas
    const totalColumns = templateSheet.columnCount || 20;
    this.logger.log(
      `Estableciendo ancho de ${this.COLUMN_WIDTH} unidades para ${totalColumns} columnas`,
    );
    ...
    for (let columnNumber = 1; columnNumber <= totalColumns; columnNumber++) {
      ...
      this.logger.log(
        `Columna ${columnNumber}: ancho establecido a ${this.COLUMN_WIDTH} unidades`,
      );
    ...
    templateSheet.eachRow((row, rowNumber) => {
      ...
      row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
        ...
        const isPlaceholder =
          typeof cell.value === 'string' && /\{\{.*\}\}/.test(cell.value);
